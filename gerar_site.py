    ...
    try:
        with open('municipios_br.csv', encoding='utf-8') as f:
            for row in csv.DictReader(f):
                _BASE_MUN[(row['nome_norm'], row['uf'].upper())] = (row['lat'], row['lon'])
    except Exception as e:
        print('  base municipios indisponivel (%s) -- mapa sem pontos' % repr(e)[:50])
    return _BASE_MUN
...
def merge_historico(novos, hoje):
    ...
    base = _base_mun()
    for r in todos:
        ll = base.get((_n(r.get('MUNICIPIO', '')), (r.get('UF', '') or '').upper()))
        if ll:
            try:
                r['lat'] = float(ll[0]); r['lon'] = float(ll[1])
            except Exception:
                pass
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .vgo{background:var(--ok);color:#fff;border-radius:8px;padding:9px 12px;margin:8px 0;font-size:.86rem;font-weight:700;line-height:1.45}
  .vgo b{color:#FEF3C7}
  ...
  .saude-erro{background:#FEE2E2;border:1px solid #EF4444;color:#991B1B;padding:11px 14px;border-radius:10px;margin:10px 0;font-size:.85rem;font-weight:600}
...
<body>
  ...
  <div class="wrap" id="app" style="display:none">
    ...
    <div id="saude" style="display:none"></div>
    ...
    <div class="sec" id="s-diario">
      <div class="panel">
        <div class="clhead"><span class="ht">Diario de execucao</span><span class="hs">auditoria das coletas do radar (quando rodou, quantos, se o PNCP truncou)</span></div>
...
function score(r){
  ...
  var d=diasAte(r),sp;
  if(d==null)sp=30;else if(d<0)sp=0;else if(d<=3)sp=55;else if(d<=30)sp=100-(d-3)*1.2;else sp=Math.max(20,70-(d-30)*0.5);
  var v=valNum(r),sv;
  if(v==null)sv=40;else if(v>=2e6)sv=100;else if(v>=5e5)sv=80;else if(v>=1e5)sv=60;else if(v>=2e4)sv=40;else sv=25;
...
function aplicar(){
  ...
  VIS=DADOS.filter(function(r){
    ...
    if(mo&&g(r,'MODALIDADE')!==mo)return false;
...
function ordenar(){
  ...
  VIS.sort(function(a,b){
    var va,vb;
    if(c==='score'){va=score(a);vb=score(b);}
    else if(c==='valor'){va=valNum(a);vb=valNum(b);va=(va==null?-1:va);vb=(vb==null?-1:vb);}
    else if(c==='data'){va=parseBR(g(a,'DATA SESSAO'));vb=parseBR(g(b,'DATA SESSAO'));va=va?va.getTime():8.64e15;vb=vb?vb.getTime():8.64e15;}  /* sem data -> fim (nao topo) */
    else if(c==='km'){va=kmNum(a);vb=kmNum(b);va=(va==null?1e9:va);vb=(vb==null?1e9:vb);}
    else if(c==='uf'){va=g(a,'UF');vb=g(b,'UF');}
    else if(c==='mun'){va=g(a,'MUNICIPIO');vb=g(b,'MUNICIPIO');}
    else if(c==='fonte'){va=g(a,'FONTE');vb=g(b,'FONTE');}
    if(va<vb)return -1*d;if(va>vb)return 1*d;return 0;
  });
  document.querySelectorAll('thead th[data-c]').forEach(function(th){th.querySelector('.ar').textContent=(th.getAttribute('data-c')===c?(d>0?'▲':'▼'):'');});
}
...
function setOrdem(v){ordCol=v;ordDir=(v==='km'||v==='data')?1:-1;pg=0;ordenar();render();}  /* data ASC = sessao mais proxima no topo */
...
function cont(arr,fn){var m={};arr.forEach(function(r){var k=fn(r);if(k==null)return;m[k]=(m[k]||0)+1;});return m;}
function opt(leg){return{responsive:true,maintainAspectRatio:false,plugins:{legend:leg?{position:'bottom',labels:{font:{size:11},boxWidth:12}}:{display:false}},scales:leg?{}:{x:{ticks:{font:{size:10}},grid:{display:false}},y:{ticks:{font:{size:10},precision:0},beginAtZero:true}}};}
function graficos(){
  var pu=cont(VIS,function(r){return g(r,'UF')||null;}),pf=cont(VIS,function(r){return g(r,'FONTE')||null;});
  var fa={'Ate 50 km':0,'51-150 km':0,'151-300 km':0,'+300 km':0,'Sem dist.':0};
...
function relicStatus(v){var mun=relicMun(v.cliente);
  if(mun&&DADOS.some(function(r){return mnorm(g(r,'MUNICIPIO'))===mun&&(capDia(r)===HOJE||diasAte(r)>=0);}))  /* so FRESCO (capturado hoje ou sessao aberta), nao historico encerrado */
...
// VEREDITO por edital (Gate DECIDIR): transforma "lista" em "acao". So aparece quando ja
// vendemos ao orgao (incumbente = P1 = onde o PGL-r mora). GO + preco-alvo (nossa mediana
// praticada) + distancia. Rotulado como referencia, nunca ordem cravada.
function vereditoAlvo(r,e){
  var km=kmNum(r), pm=e&&e.preco_med;
  var dist=(km!=null)?(' a ~'+Math.round(km)+' km da unidade'):'';
  var alvo=pm?(' Preco-alvo <b>R$ '+fnum(pm)+'/m³</b> (nossa mediana praticada aqui).'):'';
  return '<div class="vgo">&#10003; VALE DISPUTAR — ja ganhamos neste orgao '+(e.n||0)+'x'+dist+'.'+alvo
    +' <span style="font-weight:400">Confirme o edital e prepare a proposta.</span></div>';
}
function precoOrgaoHTML(r){               // "ja negociamos com este orgao" (cruza MUNICIPIO do edital)
  ...
  if(!e){var org=' '+mnorm(g(r,'ORGAO'))+' ',best='';for(var k in PRECOS_ORGAO){if(k.length>=4&&k.length>best.length&&org.indexOf(' '+k+' ')>=0){best=k;}}if(best)e=PRECOS_ORGAO[best];}  /* chave mais longa c/ fronteira de palavra vence (nao o 1o substring) */
  if(!e)return '';
  ...
  return vereditoAlvo(r,e)+'<div class="orgbox"><b>Já negociamos com este órgão</b><div class="rtxt">'+e.n+' pregão(ões) no histórico'+(e.ultimo_ano?' (última em '+esc(e.ultimo_ano)+')':'')+' · '+faixa+(e.volume?' · '+fcomp(e.volume)+' m³ contratados':'')+'<br><span style="color:#9CA3AF;font-size:.72rem">referência do que já praticamos — confira o edital atual</span></div></div>';
}
function renderHistBI(){
  carregaHist(function(){
    ...
    var ph='';(h.relicitacao_prevista||[]).forEach(function(v){
      var mun=mnorm(v.municipio||v.cliente),jaRadar=DADOS.some(function(r){return mnorm(g(r,'MUNICIPIO'))===mun&&(capDia(r)===HOJE||diasAte(r)>=0);});
...
function renderHist(){
  ...
  var rows=DADOS.filter(function(r){
    ...
    if(q){var b=(g(r,'OBJETO')+' '+g(r,'ORGAO')+' '+g(r,'MUNICIPIO')+' '+g(r,'UF')).toLowerCase();if(b.indexOf(q)<0)return false;}
    return true;
  });
  rows.sort(function(a,b){return g(b,'capturado_em').localeCompare(g(a,'capturado_em'));});
  $('hcount').textContent=rows.length+' editais no historico'+(mes?' em '+mes:'');
  var h='';rows.slice(0,600).forEach(function(r){
    var lk=g(r,'LINK'),bt=(lk.indexOf('http')===0)?'<a class="abrir" href="'+esc(lk)+'" target="_blank" rel="noopener">Abrir</a>':'-';
    h+='<tr><td style="white-space:nowrap">'+esc(g(r,'capturado_em'))+'</td><td style="white-space:nowrap">'+esc(g(r,'DATA SESSAO'))+'</td><td><span class="uf">'+esc(g(r,'UF'))+'</span></td><td>'+esc(g(r,'MUNICIPIO'))+'</td><td class="org">'+esc(g(r,'ORGAO'))+'</td><td class="obj">'+esc(g(r,'OBJETO')).slice(0,90)+'</td><td>'+esc(g(r,'FONTE'))+'</td><td>'+bt+'</td></tr>';
  });
...
function renderAnalise(){
  ...
  $('analiselist').innerHTML=h;
}
function renderDiario(){
  fetch('diario.json?v='+Date.now()).then(function(r){return r.ok?r.json():[];}).then(function(ex){
    if(!ex||!ex.length){$('diariolist').innerHTML='<div style="color:#9CA3AF;padding:14px">Sem registro de execucoes ainda.</div>';return;}
    var h='<div class="tbl-wrap"><table><thead><tr><th>Quando</th><th>Total</th><th>PNCP</th><th>Licitar</th><th>Diario Of.</th><th>Situacao</th></tr></thead><tbody>';
    ex.forEach(function(e){var al=String(e.alerta||''),ruim=/ALERTA|TRUNC|DESPENC|0/.test(al);h+='<tr><td style="white-space:nowrap">'+esc(e.quando)+'</td><td><b>'+esc(e.total)+'</b></td><td>'+esc(e.pncp)+'</td><td>'+esc(e.licitar)+'</td><td>'+esc(e.qd)+'</td><td style="font-size:.72rem;color:'+(ruim?'#C0392B':'#16A34A')+'">'+esc(al.slice(0,80))+'</td></tr>';});
    h+='</tbody></table></div>';$('diariolist').innerHTML=h;
  }).catch(function(){$('diariolist').innerHTML='<div style="color:#9CA3AF;padding:14px">Diario indisponivel.</div>';});
}
document.querySelectorAll('.tabbtn').forEach(function(b){b.addEventListener('click',function(){
  ...
  b.classList.add('on');$('s-'+b.getAttribute('data-t')).classList.add('on');
  var t=b.getAttribute('data-t');
  if(t==='mapa')setTimeout(mapa,60);
...
checkFrescor();
// Health-check: grita se a ULTIMA coleta teve alerta (fonte zerou/despencou/PNCP truncou).
...
def gerar_diario_json():
    """Le a aba 'Saude Boletim' e grava docs/diario.json (auditoria das ultimas coletas)."""
    try:
        creds = os.environ.get('GOOGLE_SHEETS_CREDENTIALS_PATH', 'credenciais/service_account.json')
        gc = gspread.service_account(filename=creds)
        vals = gc.open_by_key(SHEET_ID).worksheet('Saude Boletim').get_all_values()
        if len(vals) < 2:
            return 0
        hdr = {h.strip().upper(): i for i, h in enumerate(vals[0])}
        ...
        out = []
        for row in vals[1:]:
            out.append({'quando': cel(row, 'QUANDO'), 'total': cel(row, 'TOTAL_UNICO'),
                        'pncp': cel(row, 'PNCP_RAW'), 'licitar': cel(row, 'LICITAR_RAW'),
                        'qd': cel(row, 'QD_RAW'), 'alerta': cel(row, 'ALERTA')})
        out = out[-40:][::-1]   # ultimas 40, mais recente primeiro
        with open(os.path.join(DOCS, 'diario.json'), 'w', encoding='utf-8') as f:
            json.dump(out, f, ensure_ascii=False, separators=(',', ':'))
        return len(out)
    except Exception as e:
        print('  diario.json indisponivel: %s' % repr(e)[:80])
        return 0
